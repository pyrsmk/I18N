...
class Cli extends AbstractTranslator {
	...
	*/
    protected function _guessClientLocales(){
        ...
        // Format the locales
        foreach(explode(';', $lcs) as $lc) {
			if(strpos($lc, '=') !== false) {
            	$lc = substr($lc, strpos($lc, '=') + 1);
			}
            if($lc == 'C') {
                continue;
            }
            if(($pos = strpos($lc,'.')) !== false || ($pos = strpos($lc,'@')) !== false) {
                $lc = substr($lc, 0, $pos);
            }
            $lc = str_ireplace(array_keys($this->_languages), array_values($this->_languages), $lc);
            $lc = str_ireplace(array_keys($this->_regions), array_values($this->_regions), $lc);
            $locales[] = $lc;
        }
        // Return the locales
